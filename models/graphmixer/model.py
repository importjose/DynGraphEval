"""
GraphMixerModel: wrapper around the TPNet GraphMixer implementing the DynGraphEval BaseModel interface.

GraphMixer has NO memory bank — node state is not carried between edges.
Each query uses only the static neighbor history sampled at query time.
This makes it a strong structural baseline with minimal temporal reasoning:
the time encoder is fixed (non-trainable), so it cannot learn that recency
matters more than distant history.

Differences from TPNetTGN:
  - warmup() only builds the NeighborSampler; no memory replay needed
...
from .graphmixer_components import GraphMixer


class GraphMixerModel(BaseModel):
    """
    Wrapper for the TPNet-style GraphMixer implementing the DynGraphEval BaseModel interface.
    ...
    num_neighbors   : int      temporal neighbors per node / num_tokens (default 20)
    ...
    def __init__(
        ...
        # ── Build initial NeighborSampler (replaced in warmup) ─────────────────
        ...
        self.backbone = GraphMixer(
            ...
            num_tokens=num_neighbors,   # num_tokens = K neighbors
    ...
    def warmup(self, train_data=None, val_data=None) -> None:
        ...
        GraphMixer has no memory bank so no replay is needed — just ensure
        the neighbor sampler has access to the complete temporal graph so
    ...
    def evaluate(
        ...
        GraphMixer has no memory so there is no memory-update step after each
