"""
Train GraphMixer (TPNet implementation) on a TGB link prediction dataset.

Usage:
    python models/graphmixer/train.py --dataset tgbl-wiki
    python models/graphmixer/train.py --dataset tgbl-wiki --epochs 50 --patience 5

Checkpoint saved to:
    models/graphmixer/checkpoints/{dataset}/run0.pkl
...
def main():
    ...
    if args.gpu < 0 or not torch.cuda.is_available():
        raise SystemExit("ERROR: No CUDA GPU found. GraphMixer training requires a GPU.")
    ...
    if args.checkpoints_dir:
        ...
        ckpt_dir   = os.path.join(root_dir, "models", "graphmixer", "checkpoints", args.dataset)
    ...
    _log(f"Starting training: GraphMixer on {args.dataset} | "
    ...
    src = os.path.join(
        ...
        f"{prefix}_link_{args.dataset}_GraphMixer_seed{args.seed}.pkl"
    )
