"""
Train TGAT (TPNet implementation) on a TGB link prediction dataset.

Usage:
    python models/tgat/train.py --dataset tgbl-wiki
    python models/tgat/train.py --dataset tgbl-wiki --epochs 50 --patience 5

Checkpoint saved to:
    models/tgat/checkpoints/{dataset}/run0.pkl
...
def main():
    ...
    if args.gpu < 0 or not torch.cuda.is_available():
        raise SystemExit("ERROR: No CUDA GPU found. TGAT training requires a GPU.")
    ...
    if args.checkpoints_dir:
        ...
        ckpt_dir   = os.path.join(root_dir, "models", "tgat", "checkpoints", args.dataset)
    ...
    _log(f"Starting training: TGAT on {args.dataset} | "
